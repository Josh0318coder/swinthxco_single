...
def fast_batch_mapping(indices_a, indices_b, window_size=4):
    """優化版本的batch索引映射，支援緩存 indices_a_out"""
    device = indices_a.device
    a = indices_a.view(-1, 196)
    b = indices_b.view(-1, 196)
    ...
    if not hasattr(fast_batch_mapping, 'indices_a_cache') or \
       ...
       not torch.equal(fast_batch_mapping.cached_indices_a, indices_a):
        centers_a = ((a // 14) * 4 + 2) * 56 + ((a % 14) * 4 + 2)
    ...
    centers_b = ((b // 14) * 4 + 2) * 56 + ((b % 14) * 4 + 2)
...
def optimized_reorder_similarities(similarities, indices_a_out):
    ...
    reordered_similarities = torch.zeros(batch_size, 56*56, 16, device=device, dtype=similarities.dtype)
    ...
    def forward(
        ...
        if indices_a is None:
            ...
            if cache_key not in self.default_indices_a_cache:
                self.default_indices_a_cache[cache_key] = torch.arange(196, device=device).view(1, 1, 196, 1).expand(batch_size, 1, 196, 1)
...
# ============ Up sample for concatenate ============       
        ...
        A_feature4_1_down = A_feature4_1_down.view(batch_size, self.feature_channel, -1)
        B_feature4_1_down = B_feature4_1_down.view(batch_size, self.feature_channel, -1)
        
        '''torch.cuda.synchronize()
        start = time.time()
        memory_before = torch.cuda.memory_allocated() / 1024**2'''
        f = direct_similarity_calculation(A_feature4_1_down, B_feature4_1_down)
        if detach_flag:
            f = f.detach()
        indices_b = torch.max(f, -1, keepdim =True)[-1]
        #print(f"indices_b shape:{indices_b.shape}")
        indices_a_out, indices_b_out = fast_batch_mapping(indices_a, indices_b, 4)
        '''if self.precomputed_indices_a_out is None or self.precomputed_indices_a_out.shape[0] != batch_size or self.precomputed_indices_a_out.device != device:    
            self.precomputed_indices_a_out, _ = fast_batch_mapping(indices_a, torch.zeros_like(indices_b), 4)
        indices_a_out = self.precomputed_indices_a_out 
        _, indices_b_out = fast_batch_mapping(indices_a, indices_b, 4)'''
