...
def consistent_loss_fn(
    ...
    consistent_loss = consistent() if weight_consistent else torch.tensor(0.0, device = device)
    nonlocal_consistent_loss = nonlocal_consistent() if weight_nonlocal_consistent else torch.tensor(0.0, device = device)
...
def smoothness_loss_fn(
    ...
    smoothness_loss = smoothness() if weight_smoothness else torch.tensor(0.0,device = device)
    nonlocal_smoothness_loss = nonlocal_smoothness() if weight_nonlocal_smoothness else torch.tensor(0.0,device = device)
