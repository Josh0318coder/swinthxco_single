...
class WarpNet(nn.Module):
    ...
    def forward(
        ...
        y_raw = y.view(batch_size, channel, feature_height, feature_width)  # 2*3*44*44 (原始融合)
        similarity_map_raw = similarity_map.view(batch_size, 1, feature_height, feature_width)  # 1*44*44 (原始相似度)

        # 上采樣到176×176
        y = self.upsampling(y_raw)
        similarity_map = self.upsampling(similarity_map_raw)
        '''torch.cuda.synchronize()
        ...
        if return_raw:
            # 返回上采樣版本 + 原始44×44版本
            return y, similarity_map, y_raw, similarity_map_raw
        else:
            # 默認行為：只返回上采樣版本（保持向後兼容）
