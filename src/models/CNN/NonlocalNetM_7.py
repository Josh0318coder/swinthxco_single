...
def fast_batch_mapping(indices_a, indices_b, window_size=8):
    """優化版本的batch索引映射，支援緩存 indices_a_out"""
    device = indices_a.device
    a = indices_a.view(-1, 49)
    b = indices_b.view(-1, 49)
...
def optimized_reorder_similarities(similarities, indices_a_out):
    ...
    reordered_similarities = torch.zeros(batch_size, 56*56, 64, device=device, dtype=similarities.dtype)
    ...
    def forward(
        ...
        if indices_a is None:
            ...
            if cache_key not in self.default_indices_a_cache:
                self.default_indices_a_cache[cache_key] = torch.arange(49, device=device).view(1, 1, 49, 1).expand(batch_size, 1, 49, 1)
...
# ============ Up sample for concatenate ============       
        ...
        '''if self.precomputed_indices_a_out is None or self.precomputed_indices_a_out.shape[0] != batch_size or self.precomputed_indices_a_out.device != device:    
            self.precomputed_indices_a_out, _ = fast_batch_mapping(indices_a, torch.zeros_like(indices_b), 8)
        indices_a_out = self.precomputed_indices_a_out 
        _, indices_b_out = fast_batch_mapping(indices_a, indices_b, 8)'''
