...
parser.add_argument("--weight_contextual", type=float, default=0.002)  
parser.add_argument("--weight_perceptual", type=float, default=0.1)  
parser.add_argument("--weight_smoothness", type=float, default=500.0)  
parser.add_argument("--weight_gan", type=float, default=0.02)  
...
parser.add_argument("--epoch_train_discriminator", type=int, default=1)  
        ...
        for iter, sample in enumerate(train_progress_bar):  
            ...
            if epoch_num > opt.epoch_train_discriminator and opt.weight_gan > 0:  
                fake_data_lab = torch.cat((  
                    uncenter_l(I_current_l), uncenter_ab(I_current_ab_predict)  
                ), dim=1)  # 3通道  
